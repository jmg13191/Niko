...
class Moderation(commands.Cog):
    ...
    @commands.command(name="automod", help="Configure AutoMod settings with UI. | Konfiguriere AutoMod-Einstellungen.")
    @commands.has_permissions(manage_guild=True)
    async def automod_config(self, ctx):
        ...
        class AutoModView(discord.ui.View):
            ...
            def create_embed(self):
                ...
                desc = (
                    f"**Anti-Spam:** {'✅ Enabled' if am['antispam'] else '❌ Disabled'}\n"
                    f"**Anti-Link:** {'✅ Enabled' if am['antilink'] else '❌ Disabled'}\n"
                    f"**Bad Words:** {'✅ Enabled' if am['badwords'] else '❌ Disabled'}\n"
                    f"**Mass Mention:** {'✅ Enabled' if am['massmention'] else '❌ Disabled'}\n\n"
                    f"**Spam Threshold:** `{self.config['spam_threshold']}` msgs\n"
                    f"**Spam Interval:** `{self.config['spam_interval']}`s\n"
                    f"**Max Mentions:** `{self.config['max_mentions']}`"
                )
                return discord.Embed(title="🛡️ AutoMod Configuration", description=desc, color=discord.Color.blue())

            @discord.ui.button(label="Toggle Anti-Spam", style=discord.ButtonStyle.gray)
            async def toggle_spam(self, interaction: discord.Interaction, button: discord.ui.Button):
                self.config["automod"]["antispam"] = not self.config["automod"]["antispam"]
                self.utils.save_mod_config()
                await interaction.response.edit_message(embed=self.create_embed())

            @discord.ui.button(label="Toggle Anti-Link", style=discord.ButtonStyle.gray)
            async def toggle_link(self, interaction: discord.Interaction, button: discord.ui.Button):
                self.config["automod"]["antilink"] = not self.config["automod"]["antilink"]
                self.utils.save_mod_config()
                await interaction.response.edit_message(embed=self.create_embed())

            @discord.ui.button(label="Toggle Bad Words", style=discord.ButtonStyle.gray)
            async def toggle_words(self, interaction: discord.Interaction, button: discord.ui.Button):
                self.config["automod"]["badwords"] = not self.config["automod"]["badwords"]
                self.utils.save_mod_config()
                await interaction.response.edit_message(embed=self.create_embed())
            ...
            async def edit_settings(self, interaction: discord.Interaction, button: discord.ui.Button):
                modal = AutoModSettingsModal(self)
                await interaction.response.send_modal(modal)

        class AutoModSettingsModal(discord.ui.Modal, title="AutoMod Detailed Settings"):
            threshold = discord.ui.TextInput(label="Spam Threshold (messages)", default="6")
            interval = discord.ui.TextInput(label="Spam Interval (seconds)", default="7")
            mentions = discord.ui.TextInput(label="Max Mentions", default="5")

            def __init__(self, view):
                super().__init__()
                self.view = view
                self.threshold.default = str(view.config["spam_threshold"])
                self.interval.default = str(view.config["spam_interval"])
                self.mentions.default = str(view.config["max_mentions"])

            async def on_submit(self, interaction: discord.Interaction):
                try:
                    self.view.config["spam_threshold"] = int(self.threshold.value)
                    self.view.config["spam_interval"] = int(self.interval.value)
                    self.view.config["max_mentions"] = int(self.mentions.value)
                    self.view.utils.save_mod_config()
        ...
        view = AutoModView(utils, ctx.guild.id, config)
