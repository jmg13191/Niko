...
class JoinGiveawayView(discord.ui.View):
    def __init__(self, bot):
        # A timeout of None is required for persistent views that survive bot restarts
    ...
    async def join_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Check if giveaway exists and its status
        ...
        cursor = await self.bot.cxn.fetchone("SELECT 1 FROM participants WHERE message_id = $1 AND user_id = $2", message_id, user_id)
        if await cursor:
    ...
    async def end_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        host_id, ended, prize, winners_count = giveaway
    ...
    async def select_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        cursor = await self.bot.cxn.execute("SELECT host_id FROM giveaways WHERE message_id = ?", (interaction.message.id,))
        ...
        cursor = await self.bot.cxn.execute("SELECT user_id FROM participants WHERE message_id = ?", (interaction.message.id,))
...
class Giveaway(commands.Cog):
    ...
    async def cog_load(self):
        ...
        # Create necessary tables using same db connection (SQLite)
    ...
    async def cog_unload(self):
        # Cleanly stop the check task on unload
    ...
    async def start(self, ctx, duration: str, winners: str, *, prize: str):
        ...
        # Parse winners carefully (accepts strings like "2" or "2w")
        ...
        # Add winners label below description or as field if not implicit
    ...
    async def reroll(self, ctx, message_id: int):
        ...
        # Verify the giveaway exists and is ended
        cursor = await self.bot.cxn.execute("SELECT prize, winners_count, ended FROM giveaways WHERE message_id = ?", (message_id,))
        giveaway = await cursor.fetchone()
        
        if not giveaway:
            return await ctx.send("❌ Could not find a giveaway with that message ID. Make sure it's correct.")
            
        prize, original_winners_count, ended = giveaway
        
        if not ended:
            return await ctx.send("❌ This giveaway hasn't ended yet! You can only reroll ended giveaways.")

        # Fetch participants
        cursor = await self.bot.cxn.execute("SELECT user_id FROM participants WHERE message_id = ?", (message_id,))
        rows = await cursor.fetchall()
        participants = [row[0] for row in rows]
        ...
        # Always reroll 1 winner by default, to prevent mass tag spam
        new_winner_count = 1
        new_winners = random.sample(participants, min(new_winner_count, len(participants)))
    ...
    async def check_giveaways(self):
        ...
        try:
            ...
            # Fetch active (not ended) giveaways
            ...
            for index in range(len(giveaways)):
                message_id, channel_id, guild_id, prize, winners_count, end_time_str, host_id = giveaways[index]
                end_time = datetime.datetime.fromisoformat(end_time_str)
                
                # Check if it should end
                if now >= end_time:
                    await self.end_giveaway(message_id, channel_id, guild_id, prize, winners_count, host_id)
                    # Yield slightly so loop doesn't block concurrently
    ...
    async def end_giveaway(self, message_id, channel_id, guild_id, prize, winners_count, host_id):
        ...
        if not channel:
            try:
                ...
                return  # We don't have access to this channel anymore

        # 3. Fetch original message safely
        try:
        ...
        # 4. Pick Winners
        rows = await self.bot.cxn.fetch("SELECT user_id FROM participants WHERE message_id = ?", (message_id,))
        participants = [row[0] for row in rows]
        ...
        # Edit original message with winners
        if msg and len(msg.embeds) > 0:
            embed = msg.embeds[0]
    ...
    async def before_check_giveaways(self):
        """Wait until bot is ready before checking DB caches."""
