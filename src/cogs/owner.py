...
class OwnerCog(commands.Cog):
    ...
    async def announce(self, ctx):
        ...
        container = discord.ui.Container(
            discord.ui.TextDisplay(
                content="## **@everyone — Major Niko Update 🚀**"
            ),
            discord.ui.TextDisplay(
                content="Niko just received a **massive feature upgrade** across customization, giveaways, and image editing."
            ),
            discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
            discord.ui.TextDisplay(
                content="### **✨ Server Profile Customization**\nYou can now personalize Niko’s appearance *per server* using the **`.setprofile`** command:\n\n- **Custom server‑specific profile picture**\n- **Custom server‑specific banner**\n- **Custom server‑specific bio**\n\nPerfect for branding your community or matching seasonal themes."
            ),
            discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
            discord.ui.TextDisplay(
                content="### **🎉 Built‑In Giveaway System**\nStart a giveaway in seconds with **`.giveaway`**\n\n- Automatic timers\n- Winner selection\n- Clean CV2‑style UI\n- Fully customizable prizes and durations"
            ),
            discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
            discord.ui.TextDisplay(
                content="### **🖼️ Advanced Image Editing (Now with GIF + CV2 Support)**\nNiko now includes a full suite of image manipulation tools — all accessible through prefix commands or the **Edit Image** context menu.\n\nAvailable commands include:\n\n- **`.grayscale`** — Convert images to grayscale\n- **`.invert`** — Invert colors\n- **`.blur`** — Apply blur\n- **`.pixelate`** — Pixelate images\n- **`.deepfry`** — Extreme deepfry + glitch + scanlines\n- **`.caption`** — Add top captions (Twemoji supported)\n- **`.captionbottom`** — Add bottom captions\n- and many more!\n\nAll tools now support:\n\n- **Animated GIFs** (effects applied frame‑by‑frame)\n- **CV2 containers** (full compatibility with thumbnails + galleries)\n- **Twemoji rendering in captions**\n- **Re‑editing images generated by Niko**"
            )
        )
        view.add_item(container)
